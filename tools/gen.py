...
HEADER_TYPES = """\
# vk.types: base handle and function-pointer type aliases for core Vulkan (generated).
#
...
HEADER_ENUMS = """\
# vk.enums: core Vulkan enumerant, bitmask, and API constants (generated).
#
...
HEADER_STRUCTS = """\
# vk.structs: core Vulkan structs and unions as C-identical records (generated).
#
...
HEADER_C = """\
# vk.c: raw loaded-pointer layer for core Vulkan (generated).
#
...
HEADER_VK = """\
# vk.vk: the flat public surface of the Vulkan bindings (generated).
#
...
def gen_types(model):
    ...
    for name, c_name in model.handles:
        out.append("# {}: the {} handle.".format(name, c_name))
        out.append("pub def {}: ptr;".format(name))
    out.append("")
    for fn in model.fns:
        out.append("# {}: the PFN_vk{} callback signature.".format(fn.name, fn.name))
...
def gen_structs(model):
    ...
    for r in model.records:
        ...
        out.append("# {}: the {} {}.".format(r.name, r.cname, noun))
...
def gen_c(model):
    ...
    out.append("# GetInstanceProcAddr: resolves a command name against an instance (nil for")
    out.append("# the global commands), returning the entry point or nil. mirrors")
    out.append("# PFN_vkGetInstanceProcAddr; the bootstrap the caller provides.")
    out.append("pub def GetInstanceProcAddr: fun(ptr, *u8) ptr;")
    out.append("")
    out.append("# GetDeviceProcAddr: resolves a command name against a device, returning the")
    out.append("# entry point or nil. mirrors PFN_vkGetDeviceProcAddr; the narrowed dispatch")
    out.append("# path for device-level commands.")
    out.append("pub def GetDeviceProcAddr: fun(ptr, *u8) ptr;")
    out.append("")
    for cmd in model.commands:
        out.append("# {}: loaded pointer for the {}-level command {}.".format(cmd.name, cmd.tier, cmd.name))
    ...
    for tier, fname, loader, loader_ty, handle, doc in tiers:
        ...
        out.append("# {}: {}".format(fname, doc))
        out.append("# ---")
        out.append("# {}: the {}".format(loader, loader_doc))
        if handle:
            out.append("# {}: the created {} to resolve against".format(handle, handle))
        out.append("# ret: the number of {} commands resolved; unresolved pointers stay nil".format(tier))
        if handle:
