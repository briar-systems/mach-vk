...
### Changed
- **Breaking: the library surface moves from `src/vk.mach` to `src/lib/vk.mach`, so its full module path is now `vk.lib.vk` instead of `vk.vk`** (#53). The `[artifact.vk]` entry follows the family layout, where artifact entries sit under `src/lib/` or `src/bin/`, as `bin/example.mach` already did. A bare `use vk;` is unaffected, since it binds the default artifact's entry wherever that lives, and every other module path (`vk.c`, `vk.types`, `vk.enums`, `vk.structs`) is unchanged. Only a consumer that imports the surface by its full path, `use vk.vk;`, must change it to `use vk.lib.vk;` or to the bare `use vk;`. The example now imports `vk.lib.vk`, `tools/gen.py` writes the surface to `src/lib/vk.mach`, and `mach test . --list` collects the same 7 tests as before.
